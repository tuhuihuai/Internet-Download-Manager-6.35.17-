@ECHO OFF & CD /D %~DP0 & TITLE 绿化 www.Laomo.me 
...
reg delete HKLM\SOFTWARE\Classes\CLSID\{3C085E26-7DF6-4A34-ADA6-877D06BAE9A8} /f>NUL 2>NUL
...
reg delete HKLM\SOFTWARE\Classes\Wow6432Node\CLSID\{3C085E26-7DF6-4A34-ADA6-877D06BAE9A8} /f>NUL 2>NUL
...
:: Windows 64-Bit
If "%PROCESSOR_ARCHITECTURE%"=="AMD64" regsvr32 /s App\IDMIECC64.dll
If "%PROCESSOR_ARCHITECTURE%"=="AMD64" regsvr32 /s App\IDMGetAll64.dll
If "%PROCESSOR_ARCHITECTURE%"=="AMD64" regsvr32 /s App\downlWithIDM64.dll

:: Windows 7/8/10
If Exist "%Public%" App\idmBroker.exe -RegServer

:: Windows All
Start /Wait /B "" "%~dp0App\IDMan.exe" /rtr /isupdt /setlngid 2052 /fulllngfile idm_chn2.lng

:: Windows 7/8/10
If Exist "%Public%" App\Uninstall.exe -instdriv

:: Windows 7/8/10
If Exist "%Public%" Rundll32 setupapi.dll,InstallHinfSection DefaultInstall 128 .\App\idmwfp.inf
:: Windows XP
If Not Exist "%Public%" Rundll32 setupapi.dll,InstallHinfSection DefaultInstall 128 .\App\idmtdi.inf
...
echo set oShellLink = WshShell.CreateShortcut("%~dp0" ^& "\IDM.lnk")>>tmp.vbs
echo oShellLink.TargetPath ="%~dp0App\IDMan.exe">>tmp.vbs
echo oShellLink.WindowStyle ="1">>tmp.vbs
echo oShellLink.IconLocation = "%~dp0App\IDMan.exe">>tmp.vbs
...
mshta VBScript:Execute("Set a=CreateObject(""WScript.Shell""):Set b=a.CreateShortcut(a.SpecialFolders(""Desktop"") & ""\IDM.lnk""):b.TargetPath=""%~dp0App\IDMan.exe"":b.WorkingDirectory=""%~dp0App"":b.Save:close")
